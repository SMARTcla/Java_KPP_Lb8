...
public class ClientHandler extends Thread{
    ...
    public void run(){
        //synchronized (cardBank){
            ...
            try{
                System.out.println(appender + "Customer Service" + getSocketInfo() + " it's finished");
                socket.close();
            } catch (IOException e) {
                e.printStackTrace();
            }
    ...
    public String getSocketInfo(){
        return  socket.getInetAddress().getCanonicalHostName() + " at the port " + socket.getPort();
    }
    ...
    private void removeCard(Object operation) throws IOException {
        ...
        if (isRemove)
            out.writeObject("Card removed!");
        else
            out.writeObject("Card does not exist!");
        out.flush();
    }
